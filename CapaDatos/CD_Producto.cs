    ...
    public class CD_Producto
    {
        public List<Producto> Listar()
        {
            ...
            try
            {
                using (SqlConnection oconexion = new SqlConnection(Conexion.cn))
                {
                    ...
                    sb.AppendLine("c.IdCategoria,c.Descripcion[Descrip.Categoria],");
                    sb.AppendLine("p.Precio,p.RutaImagen,p.Activo, p.FechaRegistro,p.FechaActualizacion");
                    ...
                    using (SqlDataReader dr = cmd.ExecuteReader())
                    {
                        while (dr.Read())
                        {
                            lista.Add(new Producto()
                            {
                                ...
                                oCategoria = new Categoria() {IdCategoria = Convert.ToInt32(dr["IdCategoria"]), Descripcion = dr["Descrip.Categoria"].ToString()},
        ...
        public bool GuardarDatosImagen(Producto obj,out string Mensaje)
        {
            ...
            try
            {
                using (SqlConnection oconexion = new SqlConnection(Conexion.cn))
                {
                    string query = "update producto set RutaImagen = @rutaimagen where IdProducto = @IdProducto";
